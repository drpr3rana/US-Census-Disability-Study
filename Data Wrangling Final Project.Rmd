...
#install.packages(c('cpsR', 'ipumsr', 'tidyverse', 'table1', 'ggplot2','scales','viridis','broom','kableExtra','jtools'))
...
census_api_key("8f4b8714d231e772d84e533d364329314747e556", install = T, overwrite = TRUE)
...
# cleaning the terms for the forest plot later
...
clean_model.4 <- clean_model.4 %>%
  mutate(
    label = ifelse(term %in% names(recodeterms),
                   recodeterms[term],
                   term)  # keep original term if no mapping
  )
...
# Displaying the predictors' odds ratios in a table
```{r}
