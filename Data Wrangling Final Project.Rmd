...
#### Installing cpsR and IPUMS to pull the basic data
...
library(ggplot2)
library(viridis)
library(scales) # this package is used for percentages
...
# rename the variable names so they'll be read properly by the functions later
cpsdata.rename <- cpsdata
names(cpsdata.rename) <- make.names(names(cpsdata))
#cpsdata.rename
```


```{r}
# need to one hot encode for categories with multiple factors/use dummy variables so that they can be processed numerically

# R does it automatically, but maybe it would be worth doing it manually.
```
...
library(broom) # clean up the model and extracts the aspects of the model

# unadjusted
clean_model.1 <- tidy(mod1)  # grabs the coefficients from the model
clean_model.1 <- clean_model.1[-1, ] # remove the intercept
clean_model.1$odds_ratio <- exp(clean_model.1$estimate)
clean_model.1$lower_CI <- exp(clean_model.1$estimate - 1.96 * clean_model.1$std.error)
...
summ(mod4)
# MODEL FIT:
# χ²(31) = 37977.61, p = 0.00
# Pseudo-R² (Cragg-Uhler) = 0.23
# Pseudo-R² (McFadden) = 0.17
# AIC = 179687.00, BIC = 180025.36 
...
kable(table_data, col.names = c("Predictor", "Odds Ratio (95% CI)", "p-value"), 
  ...
  row_spec(0, bold = TRUE, background = "#f7f7f7")  # bold header row
```
